...
const DEFAULT_ANALYZER_API_URL='https://7iwa3jh6sl.execute-api.us-east-2.amazonaws.com/analyze';
const DEFAULT_CHAT_API_URL='https://7iwa3jh6sl.execute-api.us-east-2.amazonaws.com/chat';
const ANALYZER_API_URL=import.meta.env.VITE_ANALYZER_API_URL||import.meta.env.NEXT_PUBLIC_ANALYZER_API_URL||DEFAULT_ANALYZER_API_URL;
const CHAT_API_URL=import.meta.env.VITE_CHAT_API_URL||import.meta.env.NEXT_PUBLIC_CHAT_API_URL||DEFAULT_CHAT_API_URL;
...
type ChatMessage={role:'user'|'assistant';content:string;results?:RetrievalResult[];thinking?:boolean};
...
function normalizeChatAnswer(answer:string){return answer.replace(/(^|\n)\s*[-*]\s+/g,'$1• ');}
function ThinkingDots(){
 const [step,setStep]=useState(0);
 const frames=['...','..','.'];
 useEffect(()=>{const id=window.setInterval(()=>setStep(value=>(value+1)%frames.length),360);return()=>window.clearInterval(id);},[]);
 return <span className="typing-dots" aria-label="Thinking">{frames[step]}</span>;
}
...
export default function Home(){
 ...
 async function typeAssistantAnswer(answer:string){
  ...
  setChatMessages(messages=>messages.map((message,index)=>index===messages.length-1&&message.role==='assistant'?{...message,content:'',thinking:false}:message));
  const step=answer.length>900?4:answer.length>420?3:2;
  for(let i=step;i<answer.length+step;i+=step){
   const content=answer.slice(0,i);
   setChatMessages(messages=>messages.map((message,index)=>index===messages.length-1&&message.role==='assistant'?{...message,content}:message));
   await wait(14);
  }
 }
 ...
 async function askFiling(e:FormEvent){e.preventDefault();const query=chatInput.trim();if(!query||!filingChunks.length||chatBusy)return;const results=retrieveGroundedChunks(query,filingChunks,5);setChatInput('');setChatBusy(true);setChatMessages(messages=>[...messages,{role:'user',content:query},{role:'assistant',content:results.length?'':`No indexed filing passage matched "${query}". Try a filing-specific question about revenue, risks, products, debt, cash flow, or employees.`,results,thinking:Boolean(results.length)}]);if(!results.length){setChatBusy(false);return;}try{const res=await fetch(CHAT_API_URL,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query,company:report?.company,period:report?.period,chunks:results})});const data=await res.json() as ChatApiResponse;const answer=res.ok&&data.answer?data.answer:data.error||`Chat request failed with status ${res.status}.`;await typeAssistantAnswer(answer);}catch(error){await typeAssistantAnswer(error instanceof Error?error.message:'Unable to call the chat endpoint.');}finally{setChatBusy(false);}}
